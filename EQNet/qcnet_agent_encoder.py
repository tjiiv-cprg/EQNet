...
from EQNet.retention_block import RetentionTempEncoderBlock
...
class QCNetAgentEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.t_attn_layers = nn.ModuleList([
            RetentionTempEncoderBlock(hidden_dim=hidden_dim,
                                      num_head=num_heads,
    ...
    def forward(self,
        ...
        #         batch_s.append(data['agent']['batch'] + data.num_graphs * t)
        #     batch_s = torch.cat(batch_s, dim=0)
        #     # batch_pl = torch.cat([data['map_polygon']['batch'] + data.num_graphs * t
        #     #                       for t in range(self.num_historical_steps)], dim=0)
        #     batch_pl = []
        #     for t in range(self.num_historical_steps):
        #         batch_pl.append(data['map_polygon']['batch'] + data.num_graphs * t)
        ...
        for i in range(self.num_layers):
            # x_a.shape == (A, T_hist, 128)
            x_a = self.t_attn_layers[i](x_a)
    ...
    def forward_one_step(
        ...
        assert type(data) == HeteroData
        
        if len(states) == 0:
            for _ in range(self.num_layers):
                states.append(dict())
    
        # mask = data['agent']['valid_mask'][:, t:t+1].contiguous()
        pos_a = data['agent']['position'][:, t:t+1, :self.input_dim].contiguous()
        if t == 0:
            motion_vector_a = pos_a.new_zeros(data['agent']['num_nodes'], 1, self.input_dim)
        else:
            motion_vector_a = data['agent']['position'][:, t:t+1, :self.input_dim] - data['agent']['position'][:, t-1:t, :self.input_dim]
        head_a = data['agent']['heading'][:, t:t+1].contiguous()
        head_vector_a = torch.stack([head_a.cos(), head_a.sin()], dim=-1)
        # pos_pl = data['map_polygon']['position'][:, :self.input_dim].contiguous()
        # orient_pl = data['map_polygon']['orientation'].contiguous()
        if self.dataset == 'argoverse_v2':
            # length = width = height = None
            categorical_embs = [
                self.type_a_emb(data['agent']['type'].long())
            ]
        elif self.dataset == 'argoverse_v1':
            categorical_embs = None
        else:
        ...
        if self.dataset == 'argoverse_v2':
            vel = data['agent']['velocity'][:, t:t+1, :self.input_dim].contiguous()
            x_a = torch.stack(
                [torch.norm(motion_vector_a[:, :, :2], p=2, dim=-1),
                 angle_between_2d_vectors(ctr_vector=head_vector_a, nbr_vector=motion_vector_a[:, :, :2]),
                 torch.norm(vel[:, :, :2], p=2, dim=-1),
        ...
        x_a = self.x_a_emb(continuous_inputs=x_a.view(-1, x_a.size(-1)), categorical_embs=categorical_embs)
        x_a = x_a.view(-1, 1, self.hidden_dim)
        ...
        # edge_index_pl2a = radius(x=pos_s[:, :2], y=pos_pl[:, :2], r=self.pl2a_radius, max_num_neighbors=300)
        # edge_index_pl2a = edge_index_pl2a[:, mask_s[edge_index_pl2a[1]]]
        ...
        #      rel_orient_pl2a], dim=-1)
        # r_pl2a = self.r_pl2a_emb(continuous_inputs=r_pl2a, categorical_embs=None)
        # edge_index_a2a = radius_graph(x=pos_s[:, :2], r=self.a2a_radius, loop=False,
        #                               max_num_neighbors=300)
        # edge_index_a2a = subgraph(subset=mask_s, edge_index=edge_index_a2a)[0]
        # rel_pos_a2a = pos_s[edge_index_a2a[0]] - pos_s[edge_index_a2a[1]]
        # rel_head_a2a = wrap_angle(head_s[edge_index_a2a[0]] - head_s[edge_index_a2a[1]])
        ...
        for i in range(self.num_layers):
            # x_a.shape == (A, 1, 128)
            x_a = self.t_attn_layers[i](x_a, t, states[i])
            # x_a = x_a.reshape(-1, self.hidden_dim)
            # x_a = self.pl2a_attn_layers[i]((map_enc['x_pl'].transpose(0, 1).reshape(-1, self.hidden_dim), x_a), r_pl2a,
            #                                edge_index_pl2a)
            # x_a = self.a2a_attn_layers[i](x_a, r_a2a, edge_index_a2a)
            # x_a = x_a.reshape(-1, 1, self.hidden_dim)
