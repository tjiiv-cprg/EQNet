...
class MultiScaleRetention(nn.Module):
    ...
    def parallel_forward(self, qr, kr, v, mask):
        ...
        qk_mat = qk_mat / qk_mat.detach().abs().sum(dim=-1, keepdim=True).clamp(min=CLAMP_MIN, max=5e4)
        # qk_mat = qk_mat / (qk_mat.detach().abs().sum(dim=-1, keepdim=True) + EPS)
    ...
    def chunk_recurrent_forward(
        ...
        qk_mat = qk_mat * mask
        inner_scale = qk_mat.detach().abs().sum(dim=-1, keepdim=True).clamp(min=CLAMP_MIN)
        ...
        for i in range(num_chunks):
            ...
            kv_scale = kv_state.detach().abs().sum(dim=-2, keepdim=True).max(dim=-1, keepdim=True).values.clamp(min=CLAMP_MIN)
