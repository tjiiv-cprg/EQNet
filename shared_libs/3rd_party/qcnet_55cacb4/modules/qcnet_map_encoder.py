...
class QCNetMapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if dataset == 'argoverse_v2':
            ...
            self.type_pt_emb = nn.Embedding(1, hidden_dim)
            self.side_pt_emb = nn.Embedding(1, hidden_dim)
            self.type_pl_emb = nn.Embedding(3, hidden_dim)
            self.int_pl_emb = nn.Embedding(2, hidden_dim)
        else:
            raise ValueError('{} is not a valid dataset'.format(dataset))
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        if self.dataset in ['argoverse_v1', 'argoverse_v2']:
            if self.input_dim == 2:
