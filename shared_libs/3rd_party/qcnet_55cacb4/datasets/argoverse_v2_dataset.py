...
class ArgoverseV2Dataset(Dataset):
    ...
    def download(self) -> None:
        assert False, "Please check the data path"
    ...
    def _download(self) -> None:
        ...
        from common_utils.desc import desc
        desc({
            "raw_dir": self.raw_dir,
            "os.path.isdir(raw_dir)": os.path.isdir(self.raw_dir),
            "len(self.raw_file_names) == len(self))": len(self.raw_file_names) == len(self),
            "processed_dir": self.processed_dir,
            "os.path.isdir(processed_dir)": os.path.isdir(self.processed_dir),
            "len(self.processed_file_names) == len(self))": len(self.processed_file_names) == len(self),
        })
        assert False, "Please check the data path"
        self._processed_file_names = []
        self.download()
